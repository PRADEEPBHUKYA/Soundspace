...
class _CaptureBannerState extends State<CaptureBanner> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) {
    return Consumer<AudioProvider>(builder:(_,p,__){
      final authorized = p.shizuku['authorized'] ?? false;
      final c = p.capturing ? SS.green : authorized ? SS.cyan : const Color(0xFFFF9500);
      final label = p.capturing ? 'SYSTEM CAPTURE: HIGH POWER'
          : authorized ? 'SYSTEM CAPTURE: READY'
          : 'SYSTEM CAPTURE: RESTRICTED';
      final sub = p.capturing ? 'Spatializing all apps via Shizuku'
          : authorized ? 'Tap to start system-wide processing'
          : 'Tap to unlock full audio capture';
          ...
          child:Row(children:[
            AnimatedBuilder(animation:_pulse,builder:(_,__) => Container(
              ...
              decoration:BoxDecoration(shape:BoxShape.circle,
                color:c.withOpacity(p.capturing?.5+_pulse.value*.5:.9),
                boxShadow:p.capturing?[BoxShadow(color:c.withOpacity(.4*_pulse.value),blurRadius:6,spreadRadius:2)]:[]),
            )),
